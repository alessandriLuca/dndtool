...
const SixthPage = () => {
  ...
  const [maxCustomLanguages, setMaxCustomLanguages] = useState(0);
  ...
  useEffect(() => {
    ...
    const loadLanguages = async () => {
      const languages = await parseLanguagesCSV('languages.csv');
  ...
  const handleRaceChange = (event) => {
    ...
    let raceLanguages = selectedRaceData.languages || [];
    let numCustomLanguages = raceLanguages.filter(lang => typeof lang === 'number').reduce((a, b) => a + b, 0);
    ...
    if (selectedRaceData) {
      ...
      setLanguages(raceLanguages.map(lang => typeof lang === 'number' ? '' : lang));
      setMaxCustomLanguages(numCustomLanguages);
    }
  };
  ...
  const handleSubraceChange = (event) => {
    ...
    if (selectedSubraceData) {
      ...
      const raceLanguages = [...(selectedRaceData.languages || []), ...(selectedSubraceData.languages || [])];
      let numCustomLanguages = raceLanguages.filter(lang => typeof lang === 'number').reduce((a, b) => a + b, 0);
  ...
  const handleAddCustomLanguage = () => {
    if (languages.filter(lang => lang === '').length < maxCustomLanguages) {
      setLanguages([...languages, '']);
    }
  };

  const handleRemoveCustomLanguage = (index) => {
    const newLanguages = [...languages];
  ...
  const handleResetLanguages = () => {
    const selectedRaceData = races.find(r => r.name === selectedRace);
    const selectedSubraceData = subraces.find(sr => sr.name === selectedSubrace);

    let raceLanguages = selectedRaceData.languages || [];
    if (selectedSubraceData) {
      raceLanguages = [...raceLanguages, ...(selectedSubraceData.languages || [])];
    }
  ...
  const uniqueLanguages = availableLanguages.reduce((acc, lang) => {
    if (!acc.some((l) => l.Language === lang.Language)) {
      acc.push(lang);
    }
    return acc;
  }, []);
  ...
  const customLanguageOptions = () => {
    return languages
      .map((language, index) => (
        language === '' && (
          <div key={`custom-lang-${index}`} className="language-dropdown">
            <select
            ...
            >
              ...
              {uniqueLanguages
                .filter(lang => !languages.includes(lang.Language) || lang.Language === languages[index])
                .map((lang, idx) => (
                  <option key={`${lang.Language}-${idx}`} value={lang.Language}>
                    {lang.Language}
                  </option>
                ))}
            </select>
  ...
  return (
    <div className="sixth-page-container">
      ...
      <div className="form-group">
        <label htmlFor="language-select">Select Languages:</label>
        {languages.filter(lang => lang !== '').map((language, index) => (
          <div key={`fixed-lang-${index}`}>{language}</div>
        ))}
        {customLanguageOptions()}
        <button
          onClick={handleAddCustomLanguage}
          disabled={languages.filter(lang => lang === '').length >= maxCustomLanguages || languages.length >= maxCustomLanguages}
          style={{
            backgroundColor: languages.filter(lang => lang === '').length >= maxCustomLanguages || languages.length >= maxCustomLanguages ? '#e0e0e0' : '#007bff',
            color: languages.filter(lang => lang === '').length >= maxCustomLanguages || languages.length >= maxCustomLanguages ? '#a0a0a0' : '#fff',
            cursor: languages.filter(lang => lang === '').length >= maxCustomLanguages || languages.length >= maxCustomLanguages ? 'not-allowed' : 'pointer'
          }}
        >
          Add Custom Language
        </button>
